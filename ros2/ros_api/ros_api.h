...
// Maps use-cases to ros-middleware QoS-profiles
inline rmw_qos_profile_t getProfile(const std::string& profile_name) 
{
  if      (profile_name == "default") return rmw_qos_profile_default;
  else if (profile_name == "sensor")  return rmw_qos_profile_sensor_data;
  else if (profile_name == "camera")  return rmw_qos_profile_services_default;
...
class Subscriber {
 public:
  ...
  Subscriber(const std::string& topic, 
             RosAPI::Node* node_handle, 
             Method method, 
             T* obj, 
             const std::string& profile_name = "sensor") 
  {
